...
def enter_first_name(first_name):
    element = get_element_by_type(*FIRST_NAME_INPUT)
    find_element_and_perform_action(element, "input_text", first_name)

def enter_last_name(last_name):
    element = get_element_by_type(*LAST_NAME_INPUT)
    find_element_and_perform_action(element, "input_text", last_name)

def enter_dob_day(dob_day):
    element = get_element_by_type(*DOB_DAY_INPUT)
    find_element_and_perform_action(element, "input_text", str(dob_day))

def enter_dob_month(dob_month):
    element = get_element_by_type(*DOB_MONTH_INPUT)
    find_element_and_perform_action(element, "input_text", str(dob_month))

def enter_dob_year(dob_year):
    element = get_element_by_type(*DOB_YEAR_INPUT)
    find_element_and_perform_action(element, "input_text", str(dob_year))
...
def select_optional_gender(gender):
    ...
    element = get_element_by_type(*GENDER_OPTIONAL_SELECT)
    find_element_and_perform_action(element, "select_option", str(gender_value))

def enter_optional_postcode(postcode):
    element = get_element_by_type(*POSTCODE_OPTIONAL_INPUT)
    find_element_and_perform_action(element, "input_text", postcode)

def select_gender(gender):
    gender_value = GENDER_MAPPING.get(gender)
    element = get_element_by_type(*GENDER_SELECT)
    find_element_and_perform_action(element, "select_option", str(gender_value))

def enter_postcode(postcode):
    element = get_element_by_type(*POSTCODE_INPUT)
    find_element_and_perform_action(element, "input_text", postcode)

def enter_nhs_number(nhsNumber):
    element = get_element_by_type(*NHS_NUMBER_INPUT)
    wait_for_element_to_appear(element)
    find_element_and_perform_action(element, "input_text", nhsNumber)

def click_search_by_nhs_number_link():
    element = get_element_by_type(*SEARCH_BY_NHS_NUMBER_LINK)
    find_element_and_perform_action(element, "click")

def click_search_by_demographics_link():
    element = get_element_by_type(*SEARCH_BY_DEMOGRAPHICS_LINK)
    find_element_and_perform_action(element, "click")

def click_search_by_local_records_link():
    element = get_element_by_type(*SEARCH_BY_LOCAL_RECORDS_LINK)
    find_element_and_perform_action(element, "click")
...
def click_view_record():
    element = get_element_by_type(*VIEW_RECORD_LINK)
    find_element_and_perform_action(element, "click")

def check_search_for_patient_button_visible():
    element = get_element_by_type(*SEARCH_BUTTON)
...
def click_create_a_new_patient_button():
    element = get_element_by_type(*CREATE_NEW_PATIENT_BUTTON)
    find_element_and_perform_action(element, "click")

def check_error_appears_for_first_name(wait):
    element = get_element_by_type(*FIRST_NAME_INPUT_ERROR_LABEL)
    return check_element_exists(element, wait)

def check_nhs_number_error_message_text_exists(errorMessage):
    element = get_element_by_type("text", f"Error: {errorMessage}")
...
def check_last_name_error_message_text_exists():
    element = get_element_by_type(*LAST_NAME_INPUT_ERROR_LABEL)
    return check_element_exists(element)

def check_dob_error_message_text_exists():
    element = get_element_by_type(*DOB_INPUT_ERROR_LABEL)
    return check_element_exists(element)

def check_postcode_invalid_error_message_text_exists():
    element = get_element_by_type(*POSTCODE_INVALID_INPUT_ERROR_LABEL)
    return check_element_exists(element)
...
def check_patient_postcode_search_result_exists(postcode, wait):
    element = get_element_by_type("role", "cell", postcode)
    return check_element_exists(element, wait)

def check_patient_nhs_number_search_result_exists(nhsNumber, wait):
    element = get_element_by_type("role", "cell", nhsNumber)
    wait_for_element_to_appear(element)
    return check_element_exists(element, wait)

def check_patient_not_found_for_nhs_number_message_exists(nhsNumber, wait):
    element = get_element_by_type("role", "heading", f"No result found for {nhsNumber}")
    wait_for_element_to_appear(element)
    return check_element_exists(element, wait)
...
def check_search_tips_link_exists(wait):
    element = get_element_by_type(*SEARCH_TIPS_LINK)
    return check_element_exists(element, wait)
...
def check_required_field_error_appears_for_nhsNumber(wait):
    element = get_element_by_type(*NHS_NUMBER_INPUT_ERROR_LABEL)
    return check_element_exists(element, wait)

def check_record_saved_element_exists(wait):
    element = get_element_by_type(*RECORD_SAVED_DIALOGUE)
    return check_element_exists(element, wait)
